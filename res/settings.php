...
function html_validation_options_page() {
		...
		// reset settings.
	if ( isset( $_REQUEST['purge'] ) && is_numeric( $_REQUEST['purge'] ) && isset( $_REQUEST['_wpnonce'] ) && wp_verify_nonce( sanitize_key( $_REQUEST['_wpnonce'] ) ) ) {
	...
	$url = wp_nonce_url( get_site_url() . '/wp-admin/admin.php?page=html_validation/settings.php&purge=1' );

	echo '<a href="' . esc_url( $url ) . '" onclick="return confirm(\'';
...
function html_validation_add_post_editor_link( $actions, $post ) {
	...
	if ( is_array( $posttypes ) && in_array( $post->post_type, $posttypes ) ) {
		...
		$url  = wp_nonce_url( get_site_url() . '/wp-admin/admin.php?page=html_validation%2Freport.php&validate=' . esc_attr( $linkid ) );
		$link = '<a href="' . esc_url( $url ) . '">' . __( 'Validate HTML', 'html-validation' ) . '</a>';
