...
function html_validation_rest_refresh_report() {
	...
	html_validation_report_page( '1' );
}

/**
 * Display report
 *
 * @param  mixed $ajaxnonce 1 or 0 representing wether an ajax nonce was previously used.
 */
function html_validation_report_page( $ajaxnonce = '0' ) {
	global $wpdb;
	...
	if ( '0' == $ajaxnonce && ( ! isset( $_REQUEST['_wpnonce'] ) || ! wp_verify_nonce( sanitize_key( $_REQUEST['_wpnonce'] ) ) ) ) {
		return;
	}

	// process manual recheck.
	if ( isset( $_REQUEST['validate'] ) && is_numeric( $_REQUEST['validate'] ) ) {
		$validate = (int) $_REQUEST['validate'];
		$linkid   = (int) $_REQUEST['validate'];
	...
	// validate inputs.
	$errors = html_validation_check_form_values( $ajaxnonce );
	...
	if ( $results ) {
		...
		foreach ( $results as $row ) {
			...
			$url = wp_nonce_url( get_site_url() . '/wp-admin/admin.php?page=html_validation%2Freport.php&validate=' . esc_attr( $row['linkid'] ) );
			echo '<a href="' . esc_url( $url ) . '" class="html_validation_recheck" data-linkid="' . esc_attr( $row['linkid'] ) . '"><i class="fas fa-sync-alt" aria-hidden="true"></i>';
			esc_html_e( 'Recheck', 'html-validation' );
			echo '</a><br />';
			echo '<a href="' . esc_url( $validateurl ) . '" target="_blank" title="';
			esc_html_e( 'Opens in a new window', 'html-validation' );
 ...
 * @param  mixed $ajaxnonce 1 or 0 representing wether an ajax nonce was previously used.
 */
function html_validation_check_form_values( $ajaxnonce = 0 ) {
	global $wpdb;
	$error = array();

	if ( '0' == $ajaxnonce && ! isset( $_REQUEST['_wpnonce'] ) || ( isset( $_REQUEST['_wpnonce'] ) && ! wp_verify_nonce( sanitize_key( $_REQUEST['_wpnonce'] ) ) ) ) {
		return $error;
	}
